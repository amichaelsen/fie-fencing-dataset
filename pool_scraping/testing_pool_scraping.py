...
for idx, name in enumerate(names):
    print("#{i} {name:<20} (ID {id})  |".format(i=idx+1, name=name, id=IDs[idx]), end="")
    for j in range(0, len(list(names))):
        if(j==idx):
            print(" --- |",end="")
        else:
            victory_indicator = "V" if winners[idx][j]==1 else "D"
            print(" {vw}/{sc} |".format(vw=victory_indicator, sc = scores[idx][j]),end="")
    print("")
print("\n")
...
fencer_idx = 2
opponent_idx = 4
print("Showing result from single match (fencers {} and {}):".format(fencer_idx, opponent_idx))
